"""Audit fair allocation on six fresh permutations of frozen planning tasks."""
...
def audit(directory):
 ...
 parent=ROOT/'experiments/prioritized-search-results/manifest.json'
 assert hashlib.sha256(parent.read_bytes()).hexdigest()==m['parent_manifest_sha256']
 from experiments.fair_search import orders
 fresh,banks=orders(json.loads(parent.read_text()))
 ...
 rows=r['trials'];assert len(rows)==len(m['seeds'])*30*2
 summary=[];stored=[];compositions=[];families=[];paired=[]
 expected={}
 for name in ['managed-memory','transfer-manager-disabled','compositional-transfer','prioritized-search','fair-search']:
   ...
   for x in rr:
    ...
    for attempt in x['attempts']:
     ...
     for z in attempt['diagnostics']:
      key=tuple(z['parent']);assert z['child_index']==cursors.get(key,0)
 ...
 lines=['# Fair search allocation: fresh planning orders','','Six freshly frozen permutations of two existing example banks: 180 task presentations per arm. Vocabulary, scoring, expanded length five, 450,000-step search budget and memory rules are unchanged. Both arms use the same fair allocator and start each stream with empty learned libraries. These are fresh orders, not fresh examples.','', '| Seed | Arm | Solved | Total graph steps | CPU seconds | Stored methods | Mixed successes |','|---:|---|---:|---:|---:|---:|---:|']
